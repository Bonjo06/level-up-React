    ...
    return (
        <div>
            <header className="header">
                ...
                <Link to="/iniciarsesion" className="btn btn-primary w-01">Iniciar sesión</Link>
            </header>
            ...
            <section>
                <div className="card text-bg-dark mb-3">
                    <div className="card-body">
                        ...
                        <p className="card-text">Level-Up Gamer es una tienda online enfocada en la venta de productos dedicados para gamers, donde buscamos ofrecer productos de la mejor calidad a cada uno de nuestros clientes.</p>
                    </div>
                </div>
            </section>
                    ...
                    <div className="imagenes-wrapper">
                        <div className="row row-cols-1 row-cols-md-4 g-4 mb-3">
                            {products.map((product) => (
                                <div className="col" key={product.titulo}>
                                    <div className="card h-100 producto-card" onClick={() => handleCardClick(product)}>
                                        <img src={product.imagen} className="card-img-top" alt={product.titulo} />
                                        <div className="producto-overlay">
                                            <p className="overlay-texto">Ver más información</p>
                                        </div>
                                        <div className="card-body">
                                            <p><strong>{product.titulo}</strong></p>
                                            <p className="card-text">{product.descripcion.substring(0, 40)}...</p>
            ...
            <section>
                <div className="card text-bg-dark mb-3">
                    ...
                    <div className="card-body">
                        <h5 className="card-title">Atencion Lunes a Viernes 9:00 - 18:00</h5>
                        <p className="card-text">Email: LevelUpContacto@gmail.com</p>
                        <p><strong>Teléfono: +56 9 1234 5678</strong></p>
                    </div>
                </div>
            </section>
            
            {modalVisible && selectedProduct && (
                <div className="modal-container visible" onClick={closeModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <span className="modal-close" onClick={closeModal}>&times;</span>
                        <img id="modal-img" src={selectedProduct.imagen} alt="Imagen del producto" className="img-fluid mb-3 rounded" />
                        <h2 id="modal-titulo" className="mb-3">{selectedProduct.titulo}</h2>
                        <p id="modal-descripcion">{selectedProduct.descripcion}</p>
                        <p id="modal-precio" className="fw-bold fs-5">Precio: {selectedProduct.precio}</p>
                        <p id="modal-stock" className="text-white-50">Stock: {selectedProduct.stock}</p>
                        <button type="button" className="btn btn-primary w-100 mt-3">Añadir al carro</button>
                    </div>
                </div>
            )}
        </div>
